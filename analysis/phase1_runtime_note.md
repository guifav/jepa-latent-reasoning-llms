...
## Runtime entrypoints
- config/data validator: `scripts/phase1_runner.py`
- gradient-training runner: `scripts/phase1_train.py`
- training package root: `src/jepa_phase1/`

## Implemented runner capabilities
The current training path can:
1. load a run config from `configs/phase1/runs/`;
2. load JSONL view corpora from `data/*/phase1_views/`;
...
Smoke outputs:
- `tmp_smoke/out_lm/summary.json`
- `tmp_smoke/out_coupled/summary.json`
- `tmp_smoke/out_decoupled/summary.json`
...
- LM baseline, 1 step on CPU: succeeded
  ...
  - output: `tmp_smoke/out_gemma_lm_micro/summary.json`
- coupled LLM-JEPA, 1 step on CPU: succeeded
  - wall time: about `61s`
  - peak RSS: about `11.8 GB`
  - output: `tmp_smoke/out_gemma_coupled_micro/summary.json`
...
- with that redesign, a 2-stage 1-step Gemma decoupled micro-run also succeeded:
  ...
  - output: `tmp_smoke/out_gemma_decoupled_micro/summary.json`
...
- hard-wiring `scripts/eval_regex_semantics.py` as the canonical RegexEval report artifact path (the runner now reproduces the same search-based semantic rule internally, but not yet through the external report script)
